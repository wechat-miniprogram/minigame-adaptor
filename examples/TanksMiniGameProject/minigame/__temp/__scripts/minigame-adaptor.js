...
SceneManager.name2PathMap = window.__minigamePrivate.n2p; // 将对象挂在到MiniGameAdaptor
...
if (typeof NativeGlobal != 'undefined' && typeof NativeGlobal.Phys3D != 'undefined') {
  phys3d = new NativeGlobal.Phys3D.PhysSystem();
}
...
Bridge.assembly("unity-script-converter", function ($asm, globals) {
  ...
  Bridge.define("MiniGameAdaptor.Rigidbody", {
    ...
    statics: {
      methods: {
        Deserialize: function Deserialize(data, comp, context, builtContext) {
          console.log('Rigid comp', data, comp); // return comp;
          ...
          if (typeof NativeGlobal != 'undefined' && typeof NativeGlobal.Phys3D != 'undefined') {
            ...
            if (Phys3D) {
              if (s) {
                rigidBody = new Phys3D.StaticRigidbody(phys3d);
              } else {
                rigidBody = new Phys3D.DynamicRigidbody(phys3d);
                console.log('DynamicRigidbody', rigidBody);
              }

              rigidBody.position = new Phys3D.RawVec3f(pos.x, pos.y, pos.z);
              var boxCollider = new Phys3D.BoxCollider(phys3d, new Phys3D.RawVec3f(0, 0, 0), new Phys3D.RawVec3f(1, 1, 1));
              boxCollider.attachedRigidbody = rigidBody;

              boxCollider.onCollisionEnter = function (obj) {
                console.log("on collision enter:" + obj);
              };

              boxCollider.onCollisionExit = function (obj) {
                console.log("on collision exit:" + obj);
              };
            }
          }
          ...
          function render() {
            ...
            if (typeof NativeGlobal != 'undefined' && typeof NativeGlobal.Phys3D != 'undefined') {
              // console.log("rigidBody.position", rigidBody.position)
              // const vec = new MiniGameAdaptor.Vector3(rigidBody.position.x, rigidBody.position.y, rigidBody.position.z)
              try {
                entity.transform.position.x = rigidBody.position.x;
                entity.transform.position.y = rigidBody.position.y;
                entity.transform.position.z = rigidBody.position.z;
              } catch (e) {
                console.log(e);
              } // var quaternion = new engine.Quaternion(rigidBody.rotation.x, rigidBody.rotation.y, rigidBody.rotation.z, rigidBody.rotation.w);
              // var matrix = new engine.Matrix4(); 
              // matrix.makeRotationFromQuaternion(quaternion);
              // node.rotation.setFromRotationMatrix(matrix);
